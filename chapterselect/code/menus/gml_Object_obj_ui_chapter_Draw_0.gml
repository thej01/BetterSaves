if (_enabled)
{
    ...
    for (var i = 0; i < array_length(_completed_files_b); i++)
    {
        if (_completed_files_b[i] == -1)
            continue;
        var xoff = 0
        if (_completed_files[i] > 0)
            xoff = 15
        draw_sprite_ext(spr_bettersaves_star_sideb, _completed_files_b[i], x + 180 + xoff, y + 26 + (i * 12), 1, 1, 0, c_white, _alpha)
    }
}
