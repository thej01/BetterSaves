function scr_save(save_type = global.bettersaves_save_types.normal)
{
    ...
    var side_b = false;
    if (global.filechoice_route == "_b")
        side_b = true;
    fields.file_struct.side_b = side_b;
    ...
    ini_write_string(fields.ini_section, "Version", string(global.versionno));
    scr_bettersaves_save_info(fields.ini_section)
    ossafe_ini_close();
    scr_store_ura_result(global.chapter, fields.file_struct, uraboss);
    scr_store_flower_items(fields.file_struct)
    ...
    if (save_type == global.bettersaves_save_types.completion)
        scr_bettersaves_on_completion_data_made(fields.file_struct)
}
